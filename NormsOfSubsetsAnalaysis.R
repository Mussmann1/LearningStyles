...
#df=df[!df$Instructor153==df$Instructor255,seq(1,dim(df)[2])]
...
df$GradeChangeHW <- df$HWGrades153-df$HWGrades255-mean(df$HWGrades153-df$HWGrades255)
...
p=0
for (i in seq(1,15)) {
  tempmodel=NormModel(df$GradeChange,p,i)
  B=summary(tempmodel)
...
NormModel(df$GradeChangeNotTest,0,6)
